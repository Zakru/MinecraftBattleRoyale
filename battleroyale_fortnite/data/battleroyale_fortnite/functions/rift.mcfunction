...
execute as @e[type=minecraft:creeper,tag=rift,nbt={powered:1b}] at @s if entity @a[distance=..0.8,gamemode=!spectator] run scoreboard players set @s riftUsed 1
execute at @e[type=minecraft:creeper,tag=rift,nbt={powered:1b}] run effect give @a[distance=..0.8,gamemode=!spectator] minecraft:slow_falling 10000
execute at @e[type=minecraft:creeper,tag=rift,nbt={powered:1b}] run scoreboard players set @a[distance=..0.8,gamemode=!spectator] usedRift 1
execute at @e[type=minecraft:creeper,tag=rift,nbt={powered:1b}] run tp @a[distance=..0.8,gamemode=!spectator] ~ 255 ~
...
execute unless entity @e[type=minecraft:creeper,tag=rift] at @a[distance=0.9..,gamemode=!spectator,nbt={OnGround:1b},scores={usedRift=0}] unless block ~ ~ ~ minecraft:black_carpet run effect clear @p minecraft:slow_falling
